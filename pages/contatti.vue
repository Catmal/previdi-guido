<template>
  <div :style="{
  ...
  }" class="background py-6" style="position:relative">
    <div class="container p-4">
      ...
      <div class="columns">
        <div class="column">
          <section class="box section container" style="background: rgba(255,255,255,0.85); height: 100%">
            <h3 class="mb-6 has-text-primary">Come possiamo aiutarti?</h3>
            <form action="/thanks" name="contact" data-netlify="true" data-netlify-honeypot="bot-field">
              <input type="hidden" name="form-name" value="contact" />
              <b-field>
                <b-input placeholder="Nome*" type="text" name="name" v-model="submission.name"
                  validation-message="Campo nome obbligatorio" required></b-input>
              </b-field>
              <b-field>
                <b-input required placeholder="Email*" type="email" name="email"
                  validation-message="Inserisci un indirizzo email valido" v-model="submission.email"
                  @icon-right-click="submission.email = ''"></b-input>
              </b-field>
              <b-field>
                <b-input placeholder="Telefono" type="phone" name="phone" v-model="submission.phone"
                  @icon-right-click="submission.phone = ''"></b-input>
              </b-field>
              <b-field>
                <b-input required validation-message="Campo messaggio obbligatorio" placeholder="Messaggio"
                  type="textarea" name="message" v-model="submission.message"
                  @icon-right-click="submission.message = ''" />
              </b-field>
              <b-field class="my-6" validation-message="Devi accettare per poter inviare il messaggio">
                <b-checkbox required>
                  <h5>Dichiaro di aver preso visione della informativa privacy e, autorizzo il trattamento dei miei dati
                    personali, in relazione a quanto ivi indicato. Leggi Informativa *</h5>
                </b-checkbox>
              </b-field>
              <button type="submit" class="has-background-primary p-4 has-text-white rounded-borders">Invia</button>
            </form>
          </section>
        </div>
        <div class="column">
          <section class="has-text-white has-text-right has-text-left-mobile container" style=" height: 100%">
